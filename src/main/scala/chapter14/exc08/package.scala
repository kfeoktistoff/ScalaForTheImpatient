...
/**
 * Extend the tree in the preceding exercise so that each non-leaf node stores an operator in addition to the
 * child nodes. Then write a function eval that computes the value. For example, the tree
 *
 *      +
 *     /|\
 *   *  2  -
